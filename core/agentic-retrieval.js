...
export function _resolveAgenticLLMConfig(settings = {}) {
    ...
    if (provider === 'openrouter') {
        ...
        return { ok: true, provider, model, apiKey, isDirect };
    }
    ...
    if (provider === 'vllm') {
        ...
        return { ok: true, provider, model, vllmUrl, apiKey, isDirect };
    }
...
async function _callPlanner({ systemPrompt, userMessage, llmCfg, timeoutMs }) {
    ...
    if (llmCfg.provider === 'openrouter') {
        if (llmCfg.isDirect) {
            // LLM-specific override key: call OpenRouter directly.
            endpoint = 'https://openrouter.ai/api/v1/chat/completions';
            headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${llmCfg.apiKey}` };
            requestBody = body;
        } else {
            // Shared key: route through ST's proxy (reads real key server-side).
        ...
        if (llmCfg.isDirect) {
            // LLM-specific override key: call vLLM endpoint directly.
            const { buildVllmChatCompletionsUrl } = await import('./summarizer.js');
            endpoint = buildVllmChatCompletionsUrl(llmCfg.vllmUrl);
            headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${llmCfg.apiKey}` };
            requestBody = body;
        } else {
            // Shared key: route through ST's proxy.
            endpoint = '/api/backends/chat-completions/generate';
            headers = getRequestHeaders();
