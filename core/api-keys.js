...
// ─── LLM-specific key override ──────────────────────────────────────────
//
// SEPARATE LLM KEY ARCHITECTURE (post-2026-06-26):
//
// Users may want to use different API keys for embedding and LLM tasks
// (EventBase extraction, summarization, AgentMode). When an LLM-specific
// override key is present in extension_settings, VectFox makes DIRECT
// API calls to the provider instead of routing through ST's proxy, so the
// LLM key is never shared with the embedding path.
//
// Storage: plaintext in extension_settings.vectfox under:
//   - `llm_openrouter_api_key`  — OpenRouter LLM key override
//   - `llm_vllm_api_key`        — vLLM / Custom OpenAI-compatible LLM key override
//
// Fallback: when the override is empty, all calls fall back to the existing
// shared-key behavior (ST proxy + SECRET_KEYS.OPENROUTER / SECRET_KEYS.CUSTOM).
//
// Security note: stored as plaintext in settings.json, same as the pre-2026-05-26
// vLLM key was stored. Acceptable for LAN/self-hosted setups; cloud users who
// want maximum security should configure their embedding and LLM under the same
// OpenRouter/Custom account.

/**
 * Get the LLM-specific OpenRouter key override.
 * Returns the raw plaintext value from settings, or empty string if not set.
 * When non-empty, the LLM path makes direct calls to openrouter.ai instead
 * of routing through ST's chat-completions proxy.
 *
 * @param {object} [settings] - extension_settings.vectfox
 * @returns {string}
 */
export function getLLMOpenRouterApiKey(settings) {
    const v = settings?.llm_openrouter_api_key;
    return (typeof v === 'string') ? v.trim() : '';
}

/**
 * Get the LLM-specific vLLM/Custom key override.
 * Returns the raw plaintext value from settings, or empty string if not set.
 * When non-empty, the LLM path makes direct calls with this key instead
 * of routing through ST's chat-completions proxy.
 *
 * @param {object} [settings] - extension_settings.vectfox
 * @returns {string}
 */
export function getLLMCustomApiKey(settings) {
    const v = settings?.llm_vllm_api_key;
    return (typeof v === 'string') ? v.trim() : '';
}

/**
 * Resolve the effective LLM key for a given provider, with fallback.
 *
 * Returns `{ key: string, isDirect: boolean }`:
 *   - `key`      — the API key value (may be masked when isDirect=false)
 *   - `isDirect` — true when the LLM override is set and calls should go
 *                  directly to the provider (not through ST proxy)
 *
 * @param {string} provider - 'openrouter' | 'vllm'
 * @param {object} [settings] - extension_settings.vectfox
 * @returns {{ key: string, isDirect: boolean }}
 */
export function resolveLLMApiKey(provider, settings) {
    if (provider === 'openrouter') {
        const override = getLLMOpenRouterApiKey(settings);
        if (override) return { key: override, isDirect: true };
        return { key: getOpenRouterApiKey(settings), isDirect: false };
    }
    if (provider === 'vllm') {
        const override = getLLMCustomApiKey(settings);
        if (override) return { key: override, isDirect: true };
        return { key: getCustomApiKey(settings), isDirect: false };
    }
    return { key: '', isDirect: false };
}
