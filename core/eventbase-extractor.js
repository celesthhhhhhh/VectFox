...
async function _callOpenRouter(prompt, settings, windowIndex) {
    ...
    if (!apiKey) {
        throw new EventBaseFatalError(
            'EventBase: OpenRouter API key not found. Add it in EventBase / Summarize settings → OpenRouter API Key (LLM), or use the shared Embedding key.',
            'missing_api_key',
        );
    }
...
async function _callVLLM(prompt, settings, windowIndex) {
    ...
    if (!apiKey) {
        throw new EventBaseFatalError(
            'EventBase: vLLM / Custom OpenAI-compatible API key not configured. Enter it in Core → LLM Summarization → vLLM API Key (LLM).',
            'missing_api_key',
        );
    }
