...
import { getOpenRouterApiKey, getCustomApiKey, resolveLLMApiKey } from './api-keys.js';
...
export function validateLLMConfig(settings = {}) {
    ...
    if (provider === 'openrouter') {
        const { key } = resolveLLMApiKey('openrouter', settings);
        if (!key) {
...
export function getSummarizationConfigFingerprint(settings = {}) {
    ...
    if (provider === 'openrouter') {
        const { key, isDirect } = resolveLLMApiKey('openrouter', settings);
        const keySig = key ? `${key.length}:${key.slice(0, 2)}:${key.slice(-2)}` : 'missing';
        return `openrouter|${isDirect ? 'direct' : 'proxy'}|${keySig}`;
    }

    if (provider === 'vllm') {
        const url = (settings?.chat_vllm_url || '').trim();
        const { key, isDirect } = resolveLLMApiKey('vllm', settings);
        const keySig = key ? `${key.length}:${key.slice(0, 2)}:${key.slice(-2)}` : 'missing';
        return `vllm|${url}|${isDirect ? 'direct' : 'proxy'}|${keySig}`;
    }
...
// _getOpenRouterApiKey kept as alias for any remaining call sites.
...
async function _callOpenRouter(prompt, model, settings, originalLength, maxTokens = DEFAULT_MAX_TOKENS, timeoutMs = settings?.summarize_timeout_ms || DEFAULT_TIMEOUT_MS) {
    // resolveLLMApiKey returns:
    //   isDirect=true  → LLM-specific override key (plaintext), call OpenRouter directly
    //   isDirect=false → shared key (masked), route through ST proxy so server reads real key
    const { key: apiKey, isDirect } = resolveLLMApiKey('openrouter', settings);
    if (!apiKey) {
        throw new SummarizationFatalError(
            'OpenRouter API key not found. Add it in Summarize Before Store → OpenRouter API Key (LLM) or the shared Embedding key.',
            'openrouter',
            'missing_api_key'
        );
    }
    ...
    let response;
    if (isDirect) {
        // LLM-specific override: call OpenRouter directly with the plaintext key.
        ...
        // Shared key path: route through ST's proxy (reads real key server-side).
...
async function _callVLLM(prompt, model, settings, maxTokens = DEFAULT_MAX_TOKENS, timeoutMs = settings?.summarize_timeout_ms || DEFAULT_TIMEOUT_MS) {
    ...
    if (!baseUrl) {
        throw new SummarizationFatalError('vLLM URL not configured.', 'vllm', 'missing_url');
    }

    // resolveLLMApiKey: isDirect=true → plaintext LLM override key, call directly;
    //                   isDirect=false → masked shared key, route through ST proxy.
    const { key: apiKey, isDirect } = resolveLLMApiKey('vllm', settings);
    if (!apiKey) {
        throw new SummarizationFatalError(
            'vLLM / Custom OpenAI-compatible API key not configured. Enter it in Summarize Before Store → vLLM API Key (LLM) or the shared Embedding key.',
            'vllm',
            'missing_api_key'
        );
    }
    ...
    let response;
    if (isDirect) {
        // LLM-specific override: call the vLLM endpoint directly.
        const url = buildVllmChatCompletionsUrl(baseUrl);
        ...
        // Shared key: route through ST's chat-completions proxy.
        const body = {
            ..._buildBody(prompt, model, maxTokens),
