...
> - Correction (2026-06-13): persistence **does** help standard-no-plugin, contrary to an earlier note. The *probe* (`ensureVectorizationTip` → `listChunks`) can't work there (no metadata), but the tip is written by the **ingestion loop** locally — `setVectorizationTip(uuid, end + 1)` at [`core/eventbase-workflow.js:555`](core/eventbase-workflow.js#L555), where `end` is the just-extracted window's `source_window_end`, no backend read. So after the first auto-sync window the counter is correct, and persistence keeps it correct across reloads (previously standard-no-plugin reverted to the marker on every reload). The only stale window: a chat with pre-existing events but no persisted tip yet (extracted before this feature / in another install), opened before the next window fires — it shows the marker until one new window extracts, then self-heals.
> - **Orphan sweep (added 2026-06-13)** so persistence doesn't leave junk: `pruneOrphanedChatMaps(liveUuids)` ([`core/eventbase-store.js`](core/eventbase-store.js)) prunes the THREE per-chat maps (`eventbase_autosync_start_marker`, `eventbase_last_used_window_size`, `eventbase_vectorization_tip`) for chats whose UUID no longer has an EventBase/archive-event collection. `pruneOrphanedEventBaseChatMaps()` ([`core/collection-loader.js`](core/collection-loader.js)) builds the live-UUID set from the registry; it runs once after a **successful** startup discovery ([`index.js`](index.js)) and bails on an empty registry, so a transient discovery failure can't mass-wipe. This also cleans the pre-existing marker / last-window-size orphans, not just the tip. Pruned values are regenerable (re-stamp/re-probe on next ingestion), so the sweep is safe even if occasionally over-eager.
