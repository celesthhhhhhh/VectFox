                                ...
                                <div id="VectFox_summarize_settings" style="display:none; margin-top:12px;">
                                    ...
                                    <div id="VectFox_summarize_openrouter_row" style="display:none; margin-bottom:10px;">
                                        <label for="VectFox_summarize_openrouter_apikey">
                                            <small>OpenRouter API Key <span style="opacity:0.55;">(shared with Embedding)</span></small>
                                        </label>
                                        ...
                                        <small class="VectFox_hint">Stored in ST's shared slot — also used by Embedding. Leave blank to reuse it for LLM too.</small>

                                        <label for="VectFox_llm_openrouter_apikey" style="margin-top:10px; display:block;">
                                            <small>OpenRouter API Key <span style="color: var(--SmartThemeQuoteColor, #8ab4f8);">(LLM only — optional override)</span></small>
                                        </label>
                                        <input type="password" id="VectFox_llm_openrouter_apikey" class="vectfox-input"
                                            placeholder="Separate LLM key — leave blank to share the Embedding key" autocomplete="off" />
                                        <button type="button" id="VectFox_llm_openrouter_apikey_clear" class="menu_button" style="margin-top:4px; width:auto; white-space:nowrap;"><i class="fa-solid fa-trash-can"></i> Clear LLM key</button>
                                        <small class="VectFox_hint">When set, EventBase / Summarization / AgentMode use this key and call OpenRouter directly — your Embedding key stays separate. Stored in VectFox settings.</small>
                                    </div>
                                    ...
                                    <div id="VectFox_summarize_vllm_url_row" style="display:none; margin-bottom:10px;">
                                        ...
                                        <label for="VectFox_summarize_vllm_apikey" style="margin-top: 8px;">
                                            <small>vLLM API Key <span style="opacity:0.55;">(shared with Embedding)</span> <span style="opacity:0.6;">(optional)</span></small>
                                        </label>
                                        ...
                                        <small class="VectFox_hint">Shared slot — also used by Embedding. Leave blank to reuse it for LLM too.</small>

                                        <label for="VectFox_llm_vllm_apikey" style="margin-top:10px; display:block;">
                                            <small>vLLM API Key <span style="color: var(--SmartThemeQuoteColor, #8ab4f8);">(LLM only — optional override)</span></small>
                                        </label>
                                        <input type="password" id="VectFox_llm_vllm_apikey" class="vectfox-input"
                                            placeholder="Separate LLM key — leave blank to share the Embedding key" autocomplete="off" />
                                        <button type="button" id="VectFox_llm_vllm_apikey_clear" class="menu_button" style="margin-top:4px; width:auto; white-space:nowrap;"><i class="fa-solid fa-trash-can"></i> Clear LLM key</button>
                                        <small class="VectFox_hint">When set, EventBase / Summarization / AgentMode use this key and call vLLM directly — your Embedding key stays separate. Stored in VectFox settings.</small>
                                    </div>
...
function bindSettingsEvents(settings, callbacks) {
    ...
    // ─── LLM-specific OpenRouter key (override) ───────────────────────────
    // Stored as plaintext in extension_settings.vectfox.llm_openrouter_api_key.
    // When non-empty, summarizer / eventbase / agentic call OpenRouter directly
    // with this key instead of routing through ST's proxy — embedding is unaffected.
    const updateLLMOpenRouterKeyDisplay = () => {
        const savedKey = getLLMOpenRouterApiKey(settings);
        if (savedKey) {
            const masked = savedKey.length > 4
                ? '*'.repeat(Math.min(savedKey.length - 4, 8)) + savedKey.slice(-4)
                : '*'.repeat(savedKey.length);
            $('#VectFox_llm_openrouter_apikey').attr('placeholder', `LLM key saved: ${masked} (overrides shared key for LLM calls)`);
        } else {
            $('#VectFox_llm_openrouter_apikey').attr('placeholder', 'Separate LLM key — leave blank to share the Embedding key');
        }
    };
    updateLLMOpenRouterKeyDisplay();
    $('#VectFox_llm_openrouter_apikey').on('change', function() {
        const value = String($(this).val()).trim();
        settings.llm_openrouter_api_key = value;
        Object.assign(extension_settings.vectfox, settings);
        saveSettingsDebounced();
        $(this).val('');
        if (value) {
            toastr.success('OpenRouter LLM key saved — LLM calls will use this key directly (Embedding unchanged)');
        }
        updateLLMOpenRouterKeyDisplay();
    });
    $('#VectFox_llm_openrouter_apikey_clear').on('click', function() {
        if (!getLLMOpenRouterApiKey(settings)) {
            toastr.info('No LLM-specific OpenRouter key is saved.');
            return;
        }
        if (!confirm('Clear the saved OpenRouter LLM key?\n\nLLM calls will fall back to the shared Embedding key.')) return;
        settings.llm_openrouter_api_key = '';
        Object.assign(extension_settings.vectfox, settings);
        saveSettingsDebounced();
        toastr.info('OpenRouter LLM key cleared — using shared key');
        updateLLMOpenRouterKeyDisplay();
    });

    // ─── LLM-specific vLLM key (override) ────────────────────────────────
    const updateLLMVllmKeyDisplay = () => {
        const savedKey = getLLMCustomApiKey(settings);
        if (savedKey) {
            const masked = savedKey.length > 4
                ? '*'.repeat(Math.min(savedKey.length - 4, 8)) + savedKey.slice(-4)
                : '*'.repeat(savedKey.length);
            $('#VectFox_llm_vllm_apikey').attr('placeholder', `LLM key saved: ${masked} (overrides shared key for LLM calls)`);
        } else {
            $('#VectFox_llm_vllm_apikey').attr('placeholder', 'Separate LLM key — leave blank to share the Embedding key');
        }
    };
    updateLLMVllmKeyDisplay();
    $('#VectFox_llm_vllm_apikey').on('change', function() {
        const value = String($(this).val()).trim();
        settings.llm_vllm_api_key = value;
        Object.assign(extension_settings.vectfox, settings);
        saveSettingsDebounced();
        $(this).val('');
        if (value) {
            toastr.success('vLLM LLM key saved — LLM calls will use this key directly (Embedding unchanged)');
        }
        updateLLMVllmKeyDisplay();
    });
    $('#VectFox_llm_vllm_apikey_clear').on('click', function() {
        if (!getLLMCustomApiKey(settings)) {
            toastr.info('No LLM-specific vLLM key is saved.');
            return;
        }
        if (!confirm('Clear the saved vLLM LLM key?\n\nLLM calls will fall back to the shared Embedding key.')) return;
        settings.llm_vllm_api_key = '';
        Object.assign(extension_settings.vectfox, settings);
        saveSettingsDebounced();
        toastr.info('vLLM LLM key cleared — using shared key');
        updateLLMVllmKeyDisplay();
    });
